    ...
    internal sealed class HyperTypeDescriptor : CustomTypeDescriptor
    {
        ...
        private static bool TryCreatePropertyDescriptor(ref PropertyDescriptor descriptor)
        {
            try
            {
                ...
                var flag = properties.ContainsKey(property);
                lock (properties)
                {
                        ...
                        if (supportsChangeEvents)
                        {
                            ...
                            if (ei != null)
                            {
                                ...
                                mb = tb.DefineMethod(baseMethod.Name,
                                                     ...
                                                     new Type[] {typeof (object), typeof (EventHandler)});
                                il = mb.GetILGenerator();
                                il.Emit(OpCodes.Ldarg_1);
                                il.Emit(OpCodes.Castclass, property.DeclaringType);
                                il.Emit(OpCodes.Ldarg_2);
                    ...
                    Debug.WriteLine("2: " + properties.ContainsKey(property)+" "+flag);
                    PropertyDescriptor newDesc =
                        tb.CreateType().GetConstructor(new Type[] {typeof (PropertyDescriptor)})
                    ...
                    if (!properties.ContainsKey(property)) properties.Add(property, descriptor);
                    return true;
                }
            }
            catch
